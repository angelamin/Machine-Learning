...
app = Flask(__name__)
log_file_handler = TimedRotatingFileHandler(filename="log/text-recognization.log", when="D", interval=2, backupCount=2)
...
def create_ctpnnet(sess):
    ...
    saver.restore(sess,'/ctpn_side-refinement_in/output_model1/ctpn_end2end/voc_2007_trainval/VGGnet_fast_rcnn_iter_60000.ckpt')
...
def image_feed(im,boxes=None):
    ...
    # forward pass
    # if cfg.TEST.HAS_RPN:
    #     feed_dict = {net_ctpn.data: blobs['data'], net_ctpn.im_info: blobs['im_info'], net_ctpn.keep_prob: 1.0}
    return blobs,im_scales

# @app.route('/textRecognization', methods=['POST'])
def textRecogn(image_url):
    ...
    if True:
        ...
        try:
            # results = ctpn(sess_ctpn, net_ctpn, image_url)
...
def draw_boxes(img,image_name,boxes,scale):
    ...
    cv2.imwrite(os.path.join("data_ctpn/results_new_model", base_name), img)
    ...
    with sess.as_default():
        with sess.graph.as_default():
            ...
            # print(('Detection took {:.3f}s for '
                   # '{:d} object proposals').format(timer.total_time, boxes.shape[0]))
            #OCR recognition
            results = recognition(img,image_name,boxes,scale)
            return results


if __name__ == '__main__':
    # app.run(host='0.0.0.0', port=1889, debug=True)
    im_names = glob.glob(os.path.join(cfg.DATA_DIR, 'text_picture', '*.png')) + \
               glob.glob(os.path.join(cfg.DATA_DIR, 'text_picture', '*.jpg'))
